// [test] ID: V1.1-2 | Date: 2026-05-19 | Description: Token 配额工作流级集成测试（A.1 S7：Driver-only）
'use strict';
...
async function buildEnv(llmTokensPerCall = 50) {
    ...
    const agentService = {
        getAgentById: jest.fn().mockResolvedValue({ id: 'a', model: 'gpt-4', tools: [] }),
    };
    ...
    return { driver, executor, llmClient };
}
...
describe('Token 配额集成', () => {
    test('quota 充足 → 全部 SUCCESS', async () => {
        const { driver, executor } = await buildEnv(50);
    ...
    test('quota 50 + 每次 50 token → 第二个节点 STUCK', async () => {
        const { driver, executor, llmClient } = await buildEnv(50);
    ...
    test('workflow def 的 tokenQuota 字段生效', async () => {
        ...
        const { driver, executor } = await buildEnv(40);
